;(function($, undefined){
	...
	$(document).ready(function () {
		...
		$('textarea.ckeditor, textarea.ckeditor_compact').each(function(index) {
			var $this = $(this);
			...
			ck_configurationData.extraPlugins = 'xmlentities,charcount';
			...
			ck_configurationData.height = $this.height();
			ck_configurationData.width = $this.width() + 8; // add some width to make up for the margins

			// Check if this is the compact CKEditor:
			if($this.hasClass("ckeditor_compact"))
			{
				$this.parent().addClass("ck_compact");
			...
			CKEDITOR.on('instanceReady', function(ev){
				...
				// Add a border:
				$("label.ck_compact td.cke_contents").css({borderBottom: "1px solid #aaa"});
			...
			$this.removeClass('ckeditor ckeditor_compact');
